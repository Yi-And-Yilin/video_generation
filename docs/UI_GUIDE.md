...
NSFW UI is a Tkinter-based desktop application that provides a unified interface for AI-powered media generation workflows. The application uses a tabbed interface with four main sections, each dedicated to a specific type of generation task.
...
| **`Task` dropdown** | Select task type (loaded from `task_steps.csv`) |
...
- **Log File**: `nsfw_ui.log` (main application log)
...
| `nsfw_ui_state.json` | Saved UI state |
| `nsfw_ui.log` | Application log |
