...
# Test the JSON parsing logic with the actual LLM output from the log
llm_output = '''{
...
try:
    ...
    print(f'FAILED: {e}')
    print('Trying rfind fallback...')
    brace_idx = llm_output.rfind('{')
    print(f'rfind found index: {brace_idx}')
    json_to_parse = llm_output[brace_idx:]
    try:
        parsed = json.loads(json_to_parse)
        print('SUCCESS: rfind fallback worked')
    except Exception as e2:
        print(f'FAILED: {e2}')
        print('Trying find fallback...')
        brace_idx = llm_output.find('{')
        print(f'find found index: {brace_idx}')
        json_to_parse = llm_output[brace_idx:]
        try:
            parsed = json.loads(json_to_parse)
            print('SUCCESS: find fallback worked')
            print(f'Locations: {[loc["location"] for loc in parsed["locations"]]}')
